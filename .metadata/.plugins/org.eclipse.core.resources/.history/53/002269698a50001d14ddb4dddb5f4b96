...
// Les images stéréo sont réalisée par les instruments 1 et 3, ou pas realises du tout
	...
	// Les images mono sont réalisée sur n'importe quel instrument
	forall (img in Images : TY[img] == 1) 
    	(assignedTo[img,1] + assignedTo[img,2] + assignedTo[img,3]) ==  selection[img];
    	

   	// Le satéllite ne peut pas mémoriser plus d'image que sa mémoire ne le permet
   	sum(i in 0..NbImages) PM[i] * selection[i] <= PMmax;
